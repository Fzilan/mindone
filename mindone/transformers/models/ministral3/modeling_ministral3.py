...
def repeat_kv(hidden_states: ms.Tensor, n_rep: int) -> ms.Tensor:
    """
    This function repeats key/value tensors n_rep times. The hidden states go from (batch,
...
class Ministral3Attention(ms.nn.Cell):
...
class Ministral3MLP(ms.nn.Cell):
...
class Ministral3RMSNorm(ms.nn.Cell):
...
class Ministral3RotaryEmbedding(ms.nn.Cell):
