...
def eager_attention_forward(
    ...
    **kwargs: Unpack[TransformersKwargs],
    ...
    attn_weights = ms.matmul(query, key_states.transpose(2, 3)) * scaling
    if attention_mask is not None:
        causal_mask = attention_mask[:, :, :, : key_states.shape[-2]]
        attn_weights = attn_weights + causal_mask

    attn_weights = ms.ops.softmax(attn_weights, dim=-1, dtype=ms.float32).to(query.dtype)
    attn_weights = ms.ops.dropout(attn_weights, p=dropout, training=module.training)
    attn_output = ms.matmul(attn_weights, value_states)
...
def _get_llama_4_attn_scale(positions_ids: ms.Tensor, beta: float, max_position_embeddings: int) -> ms.Tensor:
    scaling = 1 + beta * ms.log(1 + ms.floor(positions_ids / max_position_embeddings))
...
class Ministral3RMSNorm(nn.Cell):
    def __init__(self, hidden_size, eps=1e-6):
        ...
        """
        super().__init__()
        self.weight = ms.Parameter(ms.ones(hidden_size))
    ...
    def construct(self, hidden_states):
        ...
        hidden_states = hidden_states * ms.rsqrt(variance + self.variance_epsilon)
...
class Ministral3RotaryEmbedding(nn.Cell):
    ...
    def compute_default_rope_parameters(
        ...
        inv_freq = 1.0 / (
            base ** (ms.arange(0, dim, 2, dtype=ms.int64).astype(ms.float32) / dim)
        )
...
class Ministral3Model(Ministral3PreTrainedModel):
    ...
    def construct(
        self,
        input_ids: Optional[ms.LongTensor] = None,
        attention_mask: Optional[ms.Tensor] = None,
        position_ids: Optional[ms.LongTensor] = None,
        past_key_values: Optional[Cache] = None,
...
class Ministral3ForCausalLM(Ministral3PreTrainedModel, GenerationMixin):
    ...
    def construct(
        ...
        past_key_values: Optional[Cache] = None,
        inputs_embeds: Optional[ms.FloatTensor] = None,
        labels: Optional[ms.LongTensor] = None,
        use_cache: Optional[bool] = None,
        cache_position: Optional[ms.LongTensor] = None,
