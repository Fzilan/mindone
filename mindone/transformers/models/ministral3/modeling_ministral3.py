...
def eager_attention_forward(
    ...
    **kwargs: Unpack[TransformersKwargs],
    ...
    attn_weights = ops.matmul(query, key_states.transpose(2, 3)) * scaling
    if attention_mask is not None:
        causal_mask = attention_mask[:, :, :, : key_states.shape[-2]]
        attn_weights = attn_weights + causal_mask

    attn_weights = ops.softmax(attn_weights, dim=-1, dtype=torch.float32).to(query.dtype)
...
def _get_llama_4_attn_scale(positions_ids: torch.Tensor, beta: float, max_position_embeddings: int) -> torch.Tensor:
    scaling = 1 + beta * ops.log(1 + ops.floor(positions_ids / max_position_embeddings))
...
class Ministral3RotaryEmbedding(nn.Cell):
    ...
    def compute_default_rope_parameters(
        ...
        inv_freq = 1.0 / (
            base ** (ops.arange(0, dim, 2, dtype=torch.int64).to(device=device, dtype=torch.float32) / dim)
        )
