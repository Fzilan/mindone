...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        >>> from mindone.diffusers import ControlNetModel, StableDiffusionXLControlNetPAGImg2ImgPipeline, AutoencoderKL
        ...
        if image.shape[1] == 4:
            ...
            if isinstance(generator, list) and len(generator) != batch_size:
                ...
                if image.shape[0] < batch_size and batch_size % image.shape[0] == 0:
                    image = ops.cat([image] * (batch_size // image.shape[0]), axis=0)
                ...
                init_latents = retrieve_latents(self.vae, self.vae.encode(image)[0], generator=generator)
    ...
    def __call__(
        ...
        Args:
            ...
            control_image (`ms.Tensor`, `PIL.Image.Image`, `np.ndarray`, `List[ms.Tensor]`, `List[PIL.Image.Image]`, `List[np.ndarray]`,:
                ...
                the type is specified as `ms.Tensor`, it is passed to ControlNet as is. `PIL.Image.Image` can also
