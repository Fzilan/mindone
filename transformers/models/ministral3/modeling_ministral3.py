...
from mindone.transformers.utils.generic import check_model_inputs
...
from mindone.transformers.integrations import use_kernel_forward_from_hub, use_kernel_func_from_hub, use_kernelized_func
from mindone.transformers.masking_utils import create_causal_mask, create_sliding_window_causal_mask
from mindone.transformers.modeling_flash_attention_utils import FlashAttentionKwargs
from mindone.transformers.modeling_layers import (
...
from mindone.transformers.utils import TransformersKwargs, auto_docstring, can_return_tuple
from mindone.transformers.utils.generic import maybe_autocast
...
def rotate_half(x):
    ...
    x1 = x[..., : x.shape[-1] // 2]
    x2 = x[..., x.shape[-1] // 2 :]
    return mindspore.ops.cat((-x2, x1), dim=-1)
...
def eager_attention_forward(
    ...
    **kwargs: Unpack[TransformersKwargs],
    ...
    attn_weights = mindspore.ops.matmul(query, key_states.transpose(2, 3)) * scaling
    if attention_mask is not None:
        causal_mask = attention_mask[:, :, :, : key_states.shape[-2]]
        attn_weights = attn_weights + causal_mask

    attn_weights = mindspore.ops.softmax(attn_weights, dim=-1, dtype=mindspore.float32).astype(query.dtype)
    attn_weights = mindspore.ops.dropout(attn_weights, p=dropout, training=self.training)
    attn_output = mindspore.ops.matmul(attn_weights, value_states)
    attn_output = attn_output.transpose(1, 2)

    return attn_output, attn_weights


def _get_llama_4_attn_scale(positions_ids: mindspore.Tensor, beta: float, max_position_embeddings: int) -> mindspore.Tensor:
    scaling = 1 + beta * mindspore.ops.log(1 + mindspore.ops.floor(positions_ids / max_position_embeddings))
    return scaling.unsqueeze(-1)


@use_kernelized_func(apply_rotary_pos_emb)
class Ministral3Attention(nn.Cell):
    ...
    def forward(
        ...
        cache_position: Optional[mindspore.int64] = None,
...
class Ministral3RMSNorm(nn.Cell):
    def __init__(self, hidden_size, eps=1e-6):
        """
        Ministral3RMSNorm is equivalent to T5LayerNorm
        """
        super().__init__()
        self.weight = mindspore.Parameter(mindspore.ops.ones(hidden_size))
        self.variance_epsilon = eps

    def forward(self, hidden_states):
        input_dtype = hidden_states.dtype
        hidden_states = hidden_states.astype(mindspore.float32)
        variance = mindspore.ops.mean(hidden_states.pow(2), axis=-1, keep_dims=True)
...
class Ministral3DecoderLayer(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        position_ids: Optional[mindspore.int64] = None,
        past_key_values: Optional[Cache] = None,
...
class Ministral3RotaryEmbedding(nn.Cell):
    ...
    def compute_default_rope_parameters(
        ...
        inv_freq = 1.0 / (
            base ** (mindspore.ops.arange(0, dim, 2, dtype=mindspore.int64).astype(device=device, dtype=mindspore.float32) / dim)
        )
        return inv_freq, attention_factor

    # No gradient decorator needed in MindSpore
    @dynamic_rope_update  # power user: used with advanced RoPE types (e.g. dynamic rope)
    def forward(self, x, position_ids):
        inv_freq_expanded = self.inv_freq[None, :, None].astype(mindspore.float32).broadcast_to(position_ids.shape[0], -1, 1).astype(x.dtype)
        position_ids_expanded = position_ids[:, None, :].astype(mindspore.float32)

        device_type = "cpu"
        with maybe_autocast(device_type=device_type, enabled=False):  # Force float32
            freqs = mindspore.ops.matmul(inv_freq_expanded.astype(mindspore.float32), position_ids_expanded).transpose(1, 2)
            emb = mindspore.ops.cat((freqs, freqs), dim=-1)
            cos = mindspore.ops.cos(emb) * self.attention_scaling
            sin = mindspore.ops.sin(emb) * self.attention_scaling
...
class Ministral3Model(Ministral3PreTrainedModel):
    ...
    def forward(
        self,
        input_ids: Optional[mindspore.int64] = None,
        attention_mask: Optional[mindspore.Tensor] = None,
        position_ids: Optional[mindspore.int64] = None,
        past_key_values: Optional[Cache] = None,
        inputs_embeds: Optional[mindspore.float32] = None,
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if cache_position is None:
            ...
            cache_position = mindspore.ops.arange(
                past_seen_tokens, past_seen_tokens + inputs_embeds.shape[1], device=inputs_embeds.dtype
            )
...
class Ministral3ForCausalLM(Ministral3PreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        past_key_values: Optional[Cache] = None,
        inputs_embeds: Optional[mindspore.float32] = None,
        labels: Optional[mindspore.int64] = None,
        use_cache: Optional[bool] = None,
        cache_position: Optional[mindspore.int64] = None,
