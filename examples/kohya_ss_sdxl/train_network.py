...
class NetworkTrainer:
    ...
    def train(self, args):
        ...
        if args.base_weights is not None:
            # base_weights が指定されている場合は、指定された重みを読み込みマージする # 如果指定了base_weights，则读取并合并指定的权重。
        ...
        # lr schedulerを用意する
        lr_scheduler = train_util.get_scheduler_fix(args, trainable_params=args.world_size)

        # 後方互換性を確保するよ
        # TODO - trainable_params in lora.py
        try:
        ...
        if args.gradient_checkpointing:
            ...
            for t_enc in text_encoders:
                ...
                if train_text_encoder:
                    for p in t_enc.text_model.embeddings.get_parameters():
                        p.requires_grad = True
        else:
            unet.set_train(False)
            for t_enc in text_encoders:
                t_enc.set_train(False)
        ...
        # TODO 实验功能：执行 fp16 训练，包括梯度 Patch PyTorch 以在 fp16 上启用渐变比例
        if args.full_fp16:
            NotImplementedError
        ...
        def save_model_hook(models, weights, output_dir):
            ...
            if is_main_process:
                ...
                for i, model in enumerate(models):
                    if not isinstance(model, type(network)):
...
class TrainStepForSDXLLoRA(TrainStep):
    ...
    def forward(
        ...
        if ops.any(ops.isnan(latents)):
            ...
            latents = ops.nan_to_num(latents, 0)
        ...
        noise, noisy_latents, timesteps, huber_c = train_util.get_noise_noisy_latents_and_timesteps(
            self.args, self.noise_scheduler, latents
        )
