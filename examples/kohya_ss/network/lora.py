...
class LoRAModule(nn.Cell):
    ...
    def __init__(
        ...
        if org_module.__class__.__name__ == "Conv2d":
            ...
            self.lora_down = nn.Conv2d(in_dim, self.lora_dim, kernel_size, stride, padding=padding, bias=False)
        ...
        self.lora_down.weight.set_data(
            initializer(XavierUniform(), self.lora_down.weight.shape, self.lora_down.weight.dtype)
        )
        self.lora_up.weight.set_data(initializer(Zero(), self.lora_up.weight.shape, self.lora_up.weight.dtype))
    ...
    def apply_to(self):
        self.org_forward = self.org_module.construct
        self.org_module.construct = self.construct
        del self.org_module
...
class LoRAInfModule(LoRAModule):
    ...
    def merge_to(self, sd, dtype, device):
        # get up/down weight
        up_weight = sd["lora_up.weight"].float()
        down_weight = sd["lora_down.weight"].float()

        # extract weight from org_module
        org_sd = {k: v for k, v in self.org_module.parameters_and_names()}
        ...
        if len(weight.shape) == 2:
            ...
            # logger.info(conved.shape, weight.shape, module.stride, module.padding)
            weight = weight + self.multiplier * conved * self.scale

        # set weight to org_module
        org_sd["weight"] = weight.to(dtype=dtype)
        # self.org_module.load_state_dict(org_sd)
        _, _ = ms.load_param_into_net(self.org_module, org_sd)
    ...
    def get_weight(self, multiplier=None):
        ...
        up_weight = self.lora_up.weight.float()
        down_weight = self.lora_down.weight.float()
    ...
    def default_forward(self, x):
        # logger.info(f"default_forward {self.lora_name} {x.shape}")
    ...
    def get_mask_for_x(self, x):
        ...
        if mask is None or len(x.shape) == 2:
            # emb_layers in SDXL doesn't have mask
            # if "emb" not in self.lora_name:
            #     print(f"mask is None for resolution {self.lora_name}, {area}, {x.shape}")
            mask_size = (1, x.shape[1]) if len(x.shape) == 2 else (1, *x.shape[1:-1], 1)
            return ops.ones(mask_size, dtype=x.dtype) / self.network.num_sub_prompts
        if len(x.shape) == 3:
            mask = ops.reshape(mask, (1, -1, 1))
        return mask
    ...
    def postp_to_q(self, x):
        ...
        query = ops.zeros((qc, x.shape[1], x.shape[2]), device=x.device, dtype=x.dtype)
        ...
        # logger.info(f"postp_to_q {self.lora_name} {x.shape} {query.shape} {self.network.num_sub_prompts}")
        return query
    ...
    def sub_prompt_forward(self, x):
        ...
        # logger.info(f"sub_prompt_forward {self.lora_name} {x.shape} {lx.shape} {emb_idx}")
    ...
    def to_out_forward(self, x):
        # logger.info(f"to_out_forward {self.lora_name} {x.shape} {self.network.is_last_network}")
        ...
        if self.network.is_last_network:
            lx = ops.zeros(
                (self.network.num_sub_prompts * self.network.batch_size, *lx1.shape[1:]),
                device=lx1.device,
                dtype=lx1.dtype,
            )
            self.network.shared[self.lora_name] = (lx, masks)

        # logger.info(f"to_out_forward {lx.shape} {lx1.shape} {self.network.sub_prompt_index} {self.network.num_sub_prompts}")
        ...
        out = ops.zeros(
            (self.network.batch_size * (3 if has_real_uncond else 2), *x.shape[1:]), device=x.device, dtype=x.dtype
        )
        ...
        for i in range(len(masks)):
            if masks[i] is None:
                masks[i] = ops.zeros_like(masks[0])

        mask = ops.cat(masks)
        mask_sum = ops.sum(mask, dim=0) + 1e-4
        for i in range(self.network.batch_size):
            # 1枚の画像ごとに処理する
            lx1 = lx[i * self.network.num_sub_prompts : (i + 1) * self.network.num_sub_prompts]
            lx1 = lx1 * mask
            lx1 = ops.sum(lx1, dim=0)
        ...
        # logger.info(f"to_out_forward {x.shape} {out.shape} {has_real_uncond}")
        return out
...
def create_network_from_weights(
    multiplier, file, vae, text_encoder, unet, weights_sd=None, for_inference=False, **kwargs
):
    # TODO
    ...
    for key, value in weights_sd.items():
        ...
        if "alpha" in key:
            ...
            # logger.info(lora_name, value.shape, dim)
...
class LoRANetwork(nn.Cell):
    ...
    def prepare_optimizer_params(self, text_encoder_lr, unet_lr, default_lr):
        ...
        def enumerate_params(loras):
            params = []
            for lora in loras:
                params.extend(lora.get_parameters())
            return params
    ...
    def save_weights(self, file, dtype, metadata):
        ...
        logger.warning("hash precalculation of ckpts not implemented")
        ...
        ms.save_checkpoint(self, file, append_dict=metadata)
    ...
    def set_current_generation(self, batch_size, num_sub_prompts, width, height, shared, ds_ratio=None):
        ...
        def resize_add(mh, mw):
            ...
            m = ops.interpolate(mask, (mh, mw), mode="bilinear")  # doesn't work in bf16
            m = m.to(dtype=dtype)
            mask_dic[mh * mw] = m
    ...
    def backup_weights(self):
        ...
        for lora in loras:
            ...
            if not hasattr(org_module, "_lora_org_weight"):
                sd = {k: v for k, v in org_module.parameters_and_names()}
                org_module._lora_org_weight = sd["weight"].clone()
    ...
    def restore_weights(self):
        ...
        for lora in loras:
            ...
            if not org_module._lora_restored:
                ...
                _, _ = ms.load_param_into_net(org_module, sd)
                org_module._lora_restored = True
